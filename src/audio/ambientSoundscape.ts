...
export class AmbientSoundscape {
  ...
  play(event: MelodyEvent, seconds: number) {
    ...
    if (event.role === 'bell') {
      this.safePlay(this.bell, event.note, now, {
        duration: Math.min(seconds * 1.8, 3.2),
        gain: event.velocity * 1.18,
        attack: 0.001,
        release: 2.3,
      })
    ...
    if (event.role === 'arp') {
      this.safePlay(this.harp, event.note, now, {
        duration: Math.min(seconds * 1.4, 2.6),
        gain: event.velocity * 0.95,
        attack: 0.004,
        release: 1.7,
      })
      return
    ...
    this.safePlay(this.piano, event.note, now, {
      duration: Math.min(seconds * 1.65, 4.5),
      gain: event.velocity * 0.95,
      attack: 0.012,
      release: 2.1,
    })
  ...
  private createNodes(context: AudioContext, style: StyleId): SoundscapeNodes {
    ...
    pianoBus.gain.value = 0.58
    bellBus.gain.value = 0.42
    padBus.gain.value = 0.5
    textureBus.gain.value = style === 'peaceful' ? 0.035 : 0.055
    delay.delayTime.value = style === 'peaceful' ? 0.72 : 0.95
    feedback.gain.value = style === 'peaceful' ? 0.34 : 0.42
    delayReturn.gain.value = style === 'peaceful' ? 0.3 : 0.38
    lowpass.type = 'lowpass'
    lowpass.frequency.value = style === 'peaceful' ? 5200 : 3400
  ...
  private playPad(event: MelodyEvent, time: number, seconds: number) {
    ...
    notes.forEach((midi, index) => {
      ...
      gain.gain.linearRampToValueAtTime((0.02 + event.texture * 0.085) / Math.max(1, notes.length), time + 0.28)
  ...
  private playChordColor(event: MelodyEvent, time: number, seconds: number) {
    ...
    colorNotes.forEach((note, index) => {
      this.safePlay(this.strings, note, time + index * 0.08, {
        duration: Math.min(seconds * 1.5, 5.5),
        gain: event.velocity * (index === 0 ? 0.5 : 0.28),
        attack: 0.18,
        release: 3.1,
      })
    })
  ...
  private playHarmony(event: MelodyEvent, time: number, seconds: number, gain: number) {
    ;(event.harmony ?? []).slice(0, event.wide ? 2 : 1).forEach((note, index) => {
      this.safePlay(this.piano, note, time + index * 0.05, {
        duration: Math.min(seconds * 1.25, 3.8),
        gain: event.velocity * gain,
        attack: 0.018,
        release: 2,
      })
    })
  ...
  private applyEventMix(event: MelodyEvent, time: number) {
    ...
    this.nodes.padBus.gain.setTargetAtTime(0.16 + event.texture * 0.68, time, 0.12)
    this.nodes.pianoBus.gain.setTargetAtTime(event.role === 'melody' ? 0.78 : event.role === 'arp' ? 0.62 : 0.34, time, 0.1)
    this.nodes.bellBus.gain.setTargetAtTime(event.role === 'bell' ? 0.82 : 0.26, time, 0.08)
    this.nodes.textureBus.gain.setTargetAtTime((this.style === 'peaceful' ? 0.018 : 0.03) + event.texture * 0.055, time, 0.34)
    this.nodes.delayReturn.gain.setTargetAtTime((this.style === 'peaceful' ? 0.16 : 0.26) + event.texture * 0.22, time, 0.2)
  }
